---
title: "Analysis of GSE123468 (Family 12)"
...
# Statistical analysis (Fam12)

The two families of dataset GSE123468 lead to family-related batch effects (see previous similarity map) due to the strong genetic similarity of the families. Therefore, we split the dataset GSE123468 into 2 datasets. In the following R workflow, family 12 will be analysed.
...
logCPM12 <- logCPM[,family == "12"]
res12 <- mod.t.test (x = logCPM12, group = group12)
colSums(res12[,c(5,6)] < 0.05)
# 1362 results for adj. p value < 5%
```

## Saving of results

```{r}
res.GSE123468_12 <- as.data.frame(res12)
res.GSE123468_12$ENSEMBLE <- ensemblIDs
res.GSE123468_12 <- res.GSE123468_12[order(res.GSE123468_12$adj.p.value),]
write.csv(res.GSE123468_12, file = "ResultsGSE123468_Fam12.csv", row.names = FALSE)
...
## Data selection
sel <- res12$adj.p.value < 0.05
selData <- logCPM[sel,family=="12"]
selData <- selData - rowMeans(selData, na.rm = TRUE)
colnames(selData) <- paste(group12)
## Color definition
...
```{r}
GeneID.UP <- entrezIDs[(res12$`difference in means` > 0 & 
                        res12$adj.p.value < 0.05)]
GeneID.DN <- entrezIDs[(res12$`difference in means` < 0 & 
                        res12$adj.p.value < 0.05)]
```
...
```{r}
go.up.123468_12 <- go.up
go.up.123468_12 <- go.up.123468_12[order(go.up.123468_12$P.Up),]
write.csv(go.up.123468_12, file = "GO.UP_GSE123468_Fam12.csv", row.names = FALSE)
save(go.up.123468_12, file = "GO.UP_GSE123468_Fam12.RData")
```
...
write.csv(go.dn.GSE123468_12, file = "GO.DN_GSE123468_Fam12.csv", row.names = FALSE)
save(go.dn.GSE123468_12, file = "GO.DN_GSE123468_Fam12.RData")
```
